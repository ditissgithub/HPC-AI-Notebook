# Chapter 6 – xCAT
## Part 1 – xCAT Architecture & Core Concepts

---

## Table of Contents

- [6.1 What is xCAT?](#61-what-is-xcat)
- [6.2 Why xCAT is Used in HPC](#62-why-xcat-is-used-in-hpc)
- [6.3 xCAT Architecture](#63-xcat-architecture)
- [6.4 xCAT Components](#64-xcat-components)
- [6.5 xCAT Database](#65-xcat-database)
- [6.6 Management Node](#66-management-node)
- [6.7 Compute Nodes](#67-compute-nodes)
- [6.8 Provisioning Flow](#68-provisioning-flow)
- [6.9 xCAT Object Model](#69-xcat-object-model)
- [6.10 Important xCAT Commands](#610-important-xcat-commands)
- [6.11 Production Perspective](#611-production-perspective)
- [6.12 Interview Questions](#612-interview-questions)
- [6.13 Part 1 Summary](#613-part-1-summary)

---

# 6.1 What is xCAT?

**xCAT (Extreme Cluster/Cloud Administration Toolkit)** is an open-source cluster management and provisioning framework.

It is commonly used to automate the lifecycle of large numbers of physical compute nodes.

xCAT can help with:

- Hardware discovery
- Operating system provisioning
- Network boot
- DHCP
- DNS
- Node configuration
- Image management
- Firmware-related operations
- Remote execution
- Cluster administration

A simple view:

```text
                    xCAT
                     │
        ┌────────────┼────────────┐
        │            │            │
        ▼            ▼            ▼
   Provisioning   Discovery   Configuration
        │            │            │
        └────────────┼────────────┘
                     ▼
                HPC Cluster
